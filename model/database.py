class GrammaticalRelation():
    ...
    def get_tour_relation(self, tour_code, city_name):
        _pred = '(s PRED DECODE)'
        _lobj = '(s LOBJ (TOUR t))'
        _from = '(s FROM (TOUR-CODE tc "{}"))'.format(tour_code)
        _to = '(s TO (CITY-NAME cn "{}"))'.format(city_name)
        return _pred + _lobj + _from + _to


    def get_dtime_relation(self, tour_code, city_code, dtime):
        _pred = '(s PRED DEPART)'
        _lobj = '(s LOBJ (TOUR-CODE tc "{}"))'.format(tour_code)
        _to = '(s TO (CITY-CODE cc "{}"))'.format(city_code)
        _at = '(s AT (DTIME d {}))'.format(dtime)
        return _pred + _lobj + _to + _at


    def get_atime_relation(self, tour_code, city_code, atime):
        _pred = '(s PRED ARRIVE)'
        _lobj = '(s LOBJ (TOUR-CODE tc "{}"))'.format(tour_code)
        _to = '(s TO (CITY-CODE cc "{}"))'.format(city_code)
    ...
    def get_runtime_relation(self, tour_code, from_city_code, to_city_code, runtime):
        _pred = '(s PRED TRAVEL)'
        _lobj = '(s LOBJ (TOUR-CODE tc "{}"))'.format(tour_code)
    ...
    def get_transport_relation(self, tour_code, transport):
        _pred = '(s PRED TRAVEL)'
        _lobj = '(s LOBJ (TOUR-CODE tc "{}"))'.format(tour_code)
